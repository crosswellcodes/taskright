...
### SMS Provider Migration (Twilio → SignalHouse) — EVALUATION / CAPTURE DONE, NO CODE (July 26, 2026)
- [ ] **Re-platform the SMS layer from Twilio to SignalHouse.** Feasibility gate passed — SignalHouse's Group→Subgroup→Brand→Campaign→Number ISV model maps ~1:1 onto our Twilio multi-tenant architecture. Full user-validated API capture (request+response shapes for auth/provisioning/numbers/messaging/webhooks, `businesses`→payload mapping, provider-neutral `sms_*` DB migration, event-driven provisioning sequence, and a **P0–P6 phased plan behind one `SmsProvider` seam**) lives in **`shared/specs/SIGNALHOUSE_API_REFERENCE.md`**. Touches ~8 backend files (`notificationService.js`, `twilioProvisioningService.js`, `routes/webhooks.js`, `routes/auth.js`, `businesses.js`, `businessService.js`, cron jobs) + the website A2P/KYC signup flow. Best done **pre-launch** (no live businesses to migrate). **Next step:** write the migration *design spec* (SmsProvider interface + per-file swap list + migration file + test plan) from the reference doc. Open decisions before provisioning work: sole-prop `entityType` mapping, new `businesses.vertical` field, `usecase` choice (`CUSTOMER_CARE` vs `MIXED`). **Supersedes the A2P live-test item below** if the switch proceeds (10DLC brand/campaign registration moves to SignalHouse).
