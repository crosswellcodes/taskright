...
**Last updated: July 5, 2026 (Session 13 — Review Requests Component 1/3: data model + backend layer — migration 020, no-auth `/api/review` endpoints, geofence-departure trigger, opt-out toggle. 108/108 backend tests. Next: Component 2, the `/review/[token]` web page.)**
...
- **Review Requests** — `shared/specs/REVIEW_REQUESTS.md`. 3-component build (data+backend → `/review` web page → CustomerDetailScreen opt-out toggle), mirroring the job-costing cadence.
  ...
  - **⚠️ NEXT STEP: Component 2 of 3 — the `/review/[token]` Next.js page** (`TaskRight-Website/src/app/review/[token]/page.tsx`, mirror `/s/[token]`). States: valid (star selector + comment + submit), already-submitted, expired, invalid — see spec "No-Auth Review Page". Backend contract is final. Component 3 = CustomerDetailScreen opt-out toggle UI.
