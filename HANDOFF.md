...
**Last updated: July 6, 2026 (Session 14 — Service Model Components 1–3 DONE → FEATURE COMPLETE: per-customer Services (data+backend, 119/119 tests) + customer-profile builder + Templates browser & "save as template". Optional C4 cleanup remains. See Pending Work.)**
...
### Service Model Overhaul (Per-Customer Services) — COMPONENTS 1–3 DONE → FEATURE COMPLETE (July 5–6, 2026)
...
- **OPTIONAL C4 cleanup (not started):** rename the `createServiceCycle`/`getServiceCyclesByBusiness`/etc. *function symbols* (+ `/service-cycles` route naming) to template vocabulary; drop the legacy `/assign-cycle` path now that the builder replaces it. Purely internal — no behavior change.
