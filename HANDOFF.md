...
### Geofence / Clock-In Transparency — 🔨 TIER A BUILT (July 23, 2026)
- **Spec:** `shared/specs/GEOFENCE_TRANSPARENCY.md`. **Tier A only** (Tiers B & C are still spec, awaiting go-ahead). **Mobile-only — no backend, no migration, no new endpoint, no test change** (backend stays **165/165**). Babel-checked; RN sim verification is the user's.
...
- **Deferred to B/C (awaiting go-ahead):** **B** — `getJobsForTeamMember` gains `autoTrackable` → My Jobs auto/manual chip (+ pass coords for first-frame). **C** — read-only `GET /team-members/:id/active-clock` derived from `geofence_events` → cross-screen "you're clocked in" banner. Background/OS-level geofencing (app closed) and owner live-view stay out of scope.
