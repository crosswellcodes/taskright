...
**Last updated: July 8, 2026 (Session 15 — Service Model Phase 2: Per-Service Task Ownership DONE → FEATURE COMPLETE. Migration 023 retired the global `tasks` table + Tasks tab; tasks now owned per-service (`service_tasks`) + per-template (`template_tasks`); service edits diff-upsert by id. 104/104 backend tests. Prior: Session 14 — Service Model C1–4 (per-customer Services). See Service Model — Phase 2.)**
