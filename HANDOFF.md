...
**Last updated: July 6, 2026 (Session 14 — Service Model Components 1 + 2 DONE: per-customer Services (data+backend, 119/119 tests) + customer-profile Add/Edit Service builder UI. Next: Component 3 — Cycles tab → Templates browser. See Pending Work.)**
...
### Service Model Overhaul (Per-Customer Services) — COMPONENTS 1 + 2 DONE (July 5–6, 2026)
...
- **NEXT STEP → Component 3:** repurpose the mobile **Cycles tab** (`ServiceCyclesScreen`) into a **Templates** browser (label + copy), add "save as template" from a Service. Optional C4 cleanup: rename `createServiceCycle`/etc. *function symbols* and drop the legacy `/assign-cycle` path (still live for backward compat).
- **Sim verification for C2 (suggested):** open a customer → Services → "+ Add Service" (try both from-scratch and "start from template"; date-based vs day-of-week per business format) → confirm it appears + generates calls; tap a Service row → edit name/tasks/price → Save; delete a Service with no completed calls (succeeds) and one with a completed call (blocked).
