...
**Last updated: July 26, 2026 (TWO threads this session. **(1) Customer App — Task Prioritization + Feedback Rating — BUILT (July 26):** removed the customer-side "time budget" model wholesale — `TaskPickerScreen` is now **all-tasks-included optional up/down prioritization** (no budget bar, no per-task minutes), with `ConfirmationScreen`/`CurrentSelectionScreen`/`HistoryScreen` restyled to ranked priority (rank badges, priority-order rendering, no `hrs`/`min`); backend dropped the now-blocking `TIME_EXCEEDED` guard (`selectedTotalHours` still stored for the business-side lifecycle/costing). Plus an **optional in-app feedback star rating** threaded through `submitFeedback` + the feedback routes to the existing `feedbacks.rating` column (in-app now matches the SMS `/review` page; `source` still distinguishes them). No migration. **180/180 backend tests** (+4; the in-app feedback endpoint had zero prior coverage). See "Customer App — Task Prioritization + Feedback Rating" below. **(2) SMS Provider Migration (Twilio → SignalHouse) — EVALUATION / CAPTURE DONE, NO CODE:** feasibility gate passed (SignalHouse's Group→Subgroup→Brand→Campaign→Number ISV model maps ~1:1 onto our Twilio multi-tenant architecture); full user-validated API capture — request+response shapes for auth/provisioning/numbers/messaging/webhooks, `businesses`→payload field mapping, provider-neutral `sms_*` DB migration plan, event-driven provisioning sequence, and a **P0–P6 phased plan behind one `SmsProvider` seam** — lives in `shared/specs/SIGNALHOUSE_API_REFERENCE.md` (registered in DOC_REGISTRY). Next step: write the migration *design spec* from that reference; open decisions before provisioning work — sole-prop `entityType`, new `businesses.vertical` field, `usecase`. See "SMS Provider Migration" under Pending Work. Prior — Geofence / Clock-In Transparency — BUILT (July 23), all three tiers: fixed the reactivation dead state in `JobDetailScreen.js` where a job with coords but `locationPermission === null` rendered neither card (and iOS `requestAuthorization` doesn't re-fire callbacks once already-authorized, stranding job #2). **Tier A** (mobile): geofence effect re-arms per `selectionCycleId` + resolves permission via `getCurrentPosition` instead of `requestAuthorization`'s callbacks; the two conditional cards collapse into one always-present "Time Tracking" card over all six `trackingMode` states (never blank) + on-the-clock elapsed timer + Enable-location/Settings path (`AppState` re-arm) + leave-while-clocked-in `beforeRemove` confirmation (A3). **Tier B**: `getJobsForTeamMember` gains `autoTrackable` (+ route now passes `isTeamAssigned`/`teamName` — a latent dead-badge fix) → My Jobs auto/manual chip. **Tier C**: read-only `GET /team-members/:id/active-clock` (`DISTINCT ON` over `geofence_events`) + `useActiveClock` hook + persistent `ClockedInBanner` above My Jobs. No migration, no writes. 176/176 backend tests (+11, `geofenceTransparency.test.js`). See "Geofence / Clock-In Transparency" below. Prior — Geocoding Reliability BUILT (July 20): fixed the team-member "No address on file — using manual tracking" quirk (address present but never geocoded). Migration 025 (`customers.geocode_attempts/attempted_at/relevance`) + `geocodeCustomer` with a relevance gate that refuses to store confident-wrong pins + reset-on-address-change + an hourly bounded retry job + an owner-visible `geocodeStatus` "couldn't map this address" note. Copy fix + one-off backfill of 8 legacy customers. 165/165 backend tests. See "Geocoding Reliability" below. Prior — Session 18: Day-of-Week Day Snapshot parity BUILT (July 17): day-of-week owners now get a "Review {Weekday} — see what's scheduled" button in service create that opens the same `ServiceDaySnapshot` review date-based owners get. Mobile-only, no backend change (reuses `getForecast` + the presentational snapshot screen). Flavor 2 (weekday aggregate) deferred. See "Day-of-Week Day Snapshot Parity" below. Prior same session — Service Call Lifecycle BUILT + Proposed Job Costing / Expected Margin extension BUILT (July 15): the proposed state now shows expected labor cost + margin — `getJobCosts` gains `proposedLabor` (hours × Σ assignee rates; group = whole-crew sum), `expectedTotalCost`, `expectedMargin*`, an `expectedLaborIncomplete` floor flag, and an `estimatedHours` fallback fix. `ServiceCallDetailScreen` shows Proposed Labor + Expected Margin pre-completion, actuals after. No migration, no new endpoint. 151/151 backend tests. Prior same session: Service Call Lifecycle BUILT: a created service's Calls are legible immediately — the per-Call detail screen shows the proposed/expected scope (default `service_tasks` menu + `total_hours` + `price_per_visit` + assignment) in flight, then switches to confirmed (customer's submitted selection) and actual (completion + labor/margin). Backend enriches `getServiceCallDetail` with a derived `lifecycleState` + resolved `tasks[]` (fixes the latent ids-render-as-"Task N" bug) + `expectedHours`/`confirmedHours`/`expectedPrice`/`scopeIsAssumed`; `CustomerDetailScreen` upcoming rows gain a lifecycle badge. No migration, no new endpoints. 144/144 backend tests (+8). See "Service Call Lifecycle" below. Prior: Session 17 — Team Labor Costing BUILT: closed the D3 gap so group-assigned Calls now record per-member labor → profitability populates on the cost side. Option A — a group assignment resolves to every team member individually; broadened the four member-facing resolvers via one shared `assertMemberAssignedToCall` gate (`team_member_id` OR `team_memberships`), reusing the per-member labor machinery unchanged. No migration. 136/136 backend tests (+15). See "Team Labor Costing" below. Prior: Session 16 — Create-Flow Team Assignment BUILT: optional person/group assignment inside the service create flow + service-level fan-out endpoint `PUT .../services/:id/assignment`; shared controlled `AssigneePicker`; validate-first-then-create; no migration.)**
...
### SMS Provider Migration (Twilio → SignalHouse) — P0–P5 BUILT, backend-green, live cutover pending (July 31, 2026)
- **Design spec + Go-Live checklist:** `shared/specs/SIGNALHOUSE_MIGRATION.md` (§10 is the single source of truth for anything needing live creds). API capture: `shared/specs/SIGNALHOUSE_API_REFERENCE.md`.
- [x] **P0 — SmsProvider seam.** `backend/src/services/sms/` — `SmsProvider` interface + `TwilioProvider` (wraps all existing Twilio code) + `index.js` factory `getProvider(capability)` with env flags `SMS_PROVIDER` (+ `_SEND`/`_INBOUND`/`_PROVISION`/`_OTP` overrides). All call-sites routed through it; deleted `twilioProvisioningService.js`. Pure indirection.
- [x] **P1 — provider-neutral DB + config.** Migration `026_sms_provider_neutral.js` (both DBs): renamed `twilio_subaccount_sid`→`sms_subgroup_id`, `twilio_phone_number`→`sms_phone_number`, `twilio_provisioning_status`→`sms_provisioning_status`, `a2p_brand_sid`→`sms_brand_id`, `a2p_campaign_sid`→`sms_campaign_id`, `messages.twilio_message_sid`→`sms_message_id`; added `businesses.vertical` (def `PROFESSIONAL`) + `businesses.sms_provider` (def `twilio`, per-business pin). **Kept `twilio_messaging_service_sid`** (TwilioProvider needs it until P6). `.env.example` added.
- [x] **P2 — `SignalHouseProvider.send`** (`@signalhousellc/sdk`; strips `+`, `enqueuedCount`/`_id` handling, `blocked`/`failed`/`dev` statuses).
- [x] **P3 — inbound webhook** (`parseInbound` for `MESSAGE_RECEIVED`, `verifyInboundSignature` via `x-webhook-secret`, `fetchInboundMedia` for `externalMediaUrls`, one Global webhook via `createInboundWebhook`; route dispatches by `subgroupId`).
- [x] **P4 — provisioning + A2P** (`provisionBusiness` subgroup+number, `registerA2P` brand w/ EIN-scrub, `handleWebhookEvent` event-driven campaign/status; §8 decisions locked: sole-prop→`PRIVATE_PROFIT`+EIN, `vertical=PROFESSIONAL`, `usecase=CUSTOMER_CARE`).
- [x] **P5 — OTP self-build** (migration `027_otp_codes.js` both DBs; hashed/expiring/attempt-limited/throttled `sendOtp`/`verifyOtp` from `SIGNALHOUSE_OTP_SENDER`; web `/signup`+`/join` unchanged — already seam-routed).
- **Tests: 216/216 backend** (`smsProviderNeutral`, `signalHouseSend`, `signalHouseInbound`, `signalHouseProvisioning`, `signalHouseOtp`). Both `task_app_db` + `task_app_test` at migration 027 (parity).
- **Live status (Jul 31):** prod token in `backend/.env`; group **`G000045V`** ("TaskRight Pro", complianceStatus `unverified`) verified via `backend/scripts/signalhouse-whoami.js`. `signalhouse-smoke.js` confirmed FREE control-plane live (getGroup, number search `data.numbers[].number`, `createSubgroup`/`deleteSubgroup`). **`createBrand` even with `mock:true` requires a funded wallet** (mock skips the $41.50 vetting + wait, not the per-op fee). Account now funded — resume live gating next.
- [ ] **NOT cut over — Twilio still default** (`SMS_PROVIDER` unset → twilio). **P6 (retire Twilio + drop `twilio_messaging_service_sid` + default `SMS_PROVIDER=signalhouse`) is pending live verification of P2–P5** (real send, inbound+MMS, provisioning→campaign, OTP) per §10 gates. **Supersedes the A2P live-test item below.**
...
- [ ] **Team member job history** — `MyJobsScreen` only ever shows upcoming work: `getJobsForTeamMember` filters `sc.service_date >= today`, so a completed job drops off the list the day after its service date (a job completed *today* still shows until tomorrow). There's no "past jobs / my completed work" view, so a member can't look back at what they finished last week (or their logged hours). Likely a new "History" tab/screen backed by a variant query (past dates and/or `status = 'completed'`, probably paginated). Surfaced July 23, 2026 while wrapping the Geofence / Clock-In Transparency work — not blocking, build if team members ask. No migration expected (reads existing `selection_cycles`/`service_completions`/`geofence_events`).
...
### Geofence / Clock-In Transparency — ✅ BUILT (July 23, 2026)
- **Spec:** `shared/specs/GEOFENCE_TRANSPARENCY.md`. All three tiers shipped. **No migration.** **176/176 backend tests** (+11, `geofenceTransparency.test.js`). Babel-checked; RN sim verification is the user's.
- **Tier A** was mobile-only (`JobDetailScreen.js`); **Tiers B & C** add read-only backend surface (no writes, no migration):
- **The problem (team-member report):** after granting location on one job, auto clock-in couldn't be re-activated on the *next* job; there was no visible status of what the geofence tool was doing; and the status vanished when leaving a Call. Root cause in `JobDetailScreen.js`: when a job had coords but `locationPermission` was still `null`, **neither** the geofence card nor the manual card rendered — a dead screen. And iOS `@react-native-community/geolocation`'s `requestAuthorization` **doesn't reliably fire either callback once authorization is already determined** (granted on job #1), so `locationPermission` stayed `null` on job #2.
- **Tier A fix (`JobDetailScreen.js`):**
  1. **Re-arm per job** — the geofence effect is keyed on `selectionCycleId`, and a dedicated effect resets `locationPermission`/`insideGeofence`/`clockedIn` to a clean state on job change, so every job re-evaluates from scratch.
  2. **Reliable permission resolution** — `armGeofence()` still calls `requestAuthorization` to trigger the OS prompt, but resolves state via **`Geolocation.getCurrentPosition`** (its success/error path *does* fire when already-authorized), driving `locationPermission` to a concrete `true`/`false` and seeding the first distance check. A 1.2 s `setTimeout` fallback covers the case where neither `requestAuthorization` callback fires.
  3. **One always-present "Time Tracking" card** replaces the two conditional cards — driven by a derived `trackingMode` covering all six §3 states (`checking`/`auto_inside`/`auto_outside`/`manual_denied`/`manual_unmapped`/`manual_noaddress`, plus the TL3 `manual_completed` clock-out case). `null` renders as "Checking location…" with a spinner. **No state renders blank.**
  4. **On-the-clock elapsed timer** ("On the clock · 0:42", 1 s interval derived from an arrival ref) answers "am I being tracked right now?" at a glance.
  5. **Recoverable location** — an "Enable location" button (`manual_denied`) opens Settings (iOS only prompts once); an `AppState` 'active' listener re-arms automatically when the member returns from Settings.
  6. **Leave-while-clocked-in confirmation (A3)** — a `navigation` `beforeRemove` interceptor asks "You're still clocked in — leaving will clock you out of {customer}" (Stay / Clock out & leave) so the silent auto-departure isn't silent. Also fixed `clockedInRef` not being reset on auto-departure (was leaving a stale flag that could post a spurious unmount departure).
- **Tier B — My Jobs auto/manual signal:** `getJobsForTeamMember` gains a derived **`autoTrackable`** boolean (`c.lat IS NOT NULL AND c.lng IS NOT NULL`; GT-B1 — a boolean, not raw coords). The `/jobs` route now also passes through `isTeamAssigned`/`teamName` (**latent bug fix** — the S17 team badge in `MyJobsScreen` read fields the route was silently dropping). Mobile: `MyJobsScreen` shows a small **"◎ Auto clock-in" / "Manual"** chip on open jobs. (Skipped the detail-screen first-frame coord pass-through: the screen shows a full-screen spinner until `getJobDetail` resolves, so there's no first frame to optimize.)
- **Tier C — cross-screen "you're clocked in" banner:** new read-only **`GET /api/team-members/:id/active-clock`** → `getActiveClockForTeamMember` (Postgres `DISTINCT ON` picks the latest `geofence_events` row per cycle, keeps those whose latest is an `arrival` with no later `departure`, returns the most recent; null when none — GT-C1 derives from the durable source, GT-C2 no new table). Mobile: `src/hooks/useActiveClock.js` (refetches on screen focus + app foreground) + `src/components/ClockedInBanner.js` (persistent green banner above My Jobs with a live elapsed timer, deep-links to the active JobDetail; GT-C3 lives at the list level so it outlives any one Call screen).
- **Out of scope / deferred:** background/OS-level geofencing (app closed — foreground `watchPosition` only); owner-side live "who's clocked in" dashboard (Tier C's endpoint is its data seed).

### Customer App — Task Prioritization + Feedback Rating — ✅ BUILT (July 26, 2026)
- **Two changes across the customer flow. No migration.** **180/180 backend tests** (+4).
- **1) Time-budget → optional prioritization (all four customer screens).** Removed the "time budget" model wholesale from the customer side. `TaskPickerScreen` rewritten: **every menu task is always included**, shown ranked 1..N with up/down arrows (no drag lib — no new native deps), reordering **optional** (default order submits fine); no time bar, no per-task minutes. `ConfirmationScreen` ("Your Priorities" — ranked, minutes/Total-Time row removed, renders in the chosen priority order via `selectedTaskIds.map`). `CurrentSelectionScreen` (blue card, tasks modal, upcoming modal, inline calendar — all stripped of "X hours available" / per-task "min"; submitted view now shows green rank badges in priority order via mapping the stored id array, not filtering the menu; neutral task-count substitutes where a card would otherwise read empty). `HistoryScreen` (rank badges, "Show priorities", no `hrs`/`min`). Priority order persists for free — `selected_tasks` JSONB preserves array order.
- **Backend (required, not cosmetic):** removed the `TIME_EXCEEDED` guard in `customerService.submitSelections` (+ its dead route handler in `customers.js`) — with every task now submitted, totals routinely exceed `total_hours`, so the guard would have rejected normal submits. `selectedTotalHours` is still computed + stored (feeds the business-side lifecycle/costing `confirmedHours`) — just never shown to the customer. Repurposed the old "exceeds limit → 400" test in `selections.test.js` to assert the over-budget selection now submits (201).
- **2) In-app feedback star rating (optional).** `FeedbackScreen` gained a 5-star selector (amber/grey, tap-again-to-clear, Poor→Excellent label), prefilled from existing feedback, sent only when set. Backend threaded `rating` through `submitFeedback` (service) + `POST`/`GET /customers/:id/feedback` (routes; validates 1–5 or 400) — the `feedbacks.rating` column (migration 020) was already there but the in-app endpoint never wrote it, so in-app now matches the SMS `/review` page. `source` stays `in_app` (default) vs `sms_request`, so the two feedback paths capture the same rating but stay distinguishable. Added `customers.test.js` feedback-rating block (+4) — this endpoint had **zero** prior coverage.
- **Mobile verification is the user's** (RN sim; no `preview_start` per memory rule) — all touched screens babel-checked clean. Deferred (customer app): CurrentSelection consolidation (three overlapping "upcoming services" surfaces), and the moot `Math.ceil` rounding of `selectedTotalHours` in Confirmation (hours now hidden customer-side).
