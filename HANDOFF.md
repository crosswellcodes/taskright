...
**Last updated: July 11, 2026 (Session 16 — Create-Flow Team Assignment BUILT: optional person/group assignment inside the service create flow + service-level fan-out endpoint `PUT .../services/:id/assignment`; shared controlled `AssigneePicker` (dispatch refactored onto it, unchanged behavior); validate-first-then-create; no migration. 121/121 backend tests. See "Create-Flow Team Assignment" below. Prior: Session 15 — Service Model Phase 2 (per-service task ownership, migration 023).)**
...
- **Deferred / next-up:** **team labor costing** (the D3 gap — group-assigned visits get no auto labor line → profitability incomplete). This feature makes the gap easier to hit (one tap group-assigns all visits); it only acknowledges it in copy. That is the next spec.
