...
**Last updated: July 5, 2026 (Session 13 — Review Requests Components 1/3 (backend, 108/108 tests) + 2/3 (the `/review/[token]` web page, build passes) DONE. Next: Component 3, the CustomerDetailScreen opt-out toggle UI.)**
...
- **Review Requests** — `shared/specs/REVIEW_REQUESTS.md`. 3-component build (data+backend → `/review` web page → CustomerDetailScreen opt-out toggle), mirroring the job-costing cadence.
  ...
  - **⚠️ NEXT STEP: Component 3 of 3 — CustomerDetailScreen opt-out toggle UI** (React Native). Surface `reviewRequestsOptedOut` on the customer detail screen with a toggle that PATCHes `/businesses/:id/customers/:customerId { reviewRequestsOptedOut }`. Backend + web page are done; this is the last piece. Note: `getCustomerDetails` may need to also return `reviewRequestsOptedOut` if the detail payload doesn't already expose it (verify — likely the same additive-field pattern as Components 1/2 of job costing).
