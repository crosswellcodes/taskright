...
**Status:** ✅ **BUILT (July 23, 2026)** — all three tiers shipped. Mobile-heavy; one small backend addition for the list tier, one read-only endpoint for the persistent-banner tier. **No migration.** **176/176 backend tests** (+11, `geofenceTransparency.test.js`). Descends from a team-member report: after granting location on one job, the tracking status can't be re-activated on the *next* job, there's no visible indication of what the geofence tool is doing, and the status disappears entirely when leaving a Call.

**Tier B & C build note (July 23, 2026):** **Tier B** — `getJobsForTeamMember` gains a derived **`autoTrackable`** boolean (`c.lat IS NOT NULL AND c.lng IS NOT NULL`; GT-B1 — a boolean, not raw coords); the `/jobs` route now also passes through `isTeamAssigned`/`teamName` (were silently dropped — the S17 team badge was reading fields the API never sent). `MyJobsScreen` renders a small **"◎ Auto clock-in" / "Manual"** chip on open jobs. (The detail-screen first-frame coord pass-through was skipped — the screen shows a full-screen spinner until `getJobDetail` resolves, so there's no first frame to optimize.) **Tier C** — new read-only **`GET /api/team-members/:id/active-clock`** → `getActiveClockForTeamMember` (Postgres `DISTINCT ON` picks the latest event per cycle, keeps those whose latest is an `arrival`, returns the most recent; GT-C1 derives from `geofence_events`, GT-C2 no new table). Mobile: a focused `useActiveClock` hook (refetches on screen focus + app foreground) feeds a persistent green **`ClockedInBanner`** above My Jobs ("You're clocked into {customer} · 1:12 — Open ›") that deep-links to the active JobDetail (GT-C3 lives at the list level so it outlives any one Call screen). Backend-touching but **no migration, no writes**; `geofenceTransparency.test.js` covers autoTrackable (coords present/absent + route pass-through), and active-clock (open on arrival, null after departure, ignores other members', route shape + 403).

**Tier A build note (July 23, 2026):** `JobDetailScreen.js` only — the reactivation dead state is fixed. The geofence effect now re-arms per `selectionCycleId` (resets `locationPermission` on job change) and resolves permission via `Geolocation.getCurrentPosition` (reliable when already-authorized on iOS, unlike `requestAuthorization`'s callbacks; a 1.2 s fallback covers the case where neither callback fires). The two conditional cards are replaced by **one always-present "Time Tracking" card** driven by a derived `trackingMode` covering all six §3 states — `null` renders as "Checking location…", never blank. Added: an on-the-clock elapsed timer ("On the clock · 0:42"), an "Enable location" path (opens Settings; an `AppState` 'active' listener re-arms on return), and a leave-while-clocked-in confirmation (A3, via a `beforeRemove` navigation interceptor). Also reset `clockedInRef` on auto-departure (prevents a spurious unmount departure). **No backend, no migration, no new endpoint.** Babel-checked; RN sim verification is the user's.
