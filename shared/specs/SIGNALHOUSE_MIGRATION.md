...
- [~] **P4 provisioning response field paths** — CONFIRMED LIVE (smoke, Jul 31): `subRes.data.subgroupId` ✅ (`S0000AWT`) and `searchRes.data.numbers[0].number` ✅ (digits-only 11-digit, e.g. `19713120983`; `_toWireNumber`/`_toE164` handle it). `deleteSubgroup({id:subgroupId})` ✅. STILL TO CONFIRM (needs mock-brand run): `registerA2P` reads `brandRes.data.status`/`_id`; `_createCampaign` reads `res.data.campaignId`.
