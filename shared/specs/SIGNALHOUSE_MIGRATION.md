...
- [~] **P4 provisioning response field paths** — CONFIRMED LIVE (funded smoke, Jul 31): `subRes.data.subgroupId` ✅ (`S…`); `searchRes.data.numbers[0].number` ✅ (digits-only 11-digit, e.g. `19713120983`; `_toWireNumber`/`_toE164` handle it); `deleteSubgroup({id:subgroupId})` ✅; **`registerA2P` `brandRes.data._id`/`.status` ✅** (mock brand → `PENDING_CREATION`, then getBrands poll → `PENDING_APPROVAL` + `brandId` arrives async; brand ACCEPTS `referenceId`). **FIXED (bug found live): `_createCampaign` must NOT send `referenceId`** — SignalHouse rejects it on campaign create ("Unrecognized key") though it's valid on brand; removed from provider + smoke. Campaign events correlate via `subgroupId` fallback. **`purchasePhoneNumber` + `getPhoneNumbers({subgroupId})` poll CONFIRMED LIVE** (paid smoke: number provisioned to subgroup, status `PENDING`; released cleanly on cleanup). **`createCampaign` still cannot be validated with a mock brand** — a mock brand stalls at `PENDING_APPROVAL` and campaign create requires a **VERIFIED** brand ("Unable to create campaign for brand with status PENDING_APPROVAL"). This surfaced a **fixed timing bug**: `handleWebhookEvent` now creates the campaign on `BRAND_IDENTITY_STATUS_UPDATED`→`VERIFIED`/`VETTED_VERIFIED` (not on `BRAND_CREATION_SUCCESSFUL`, which only stores `sms_brand_id`). Confirming `res.data.campaignId` + a real send now **requires a real (non-mock) verified brand = actual carrier vetting** (~$41.50 + wait) — deferred until the user can do non-mock vetting.
