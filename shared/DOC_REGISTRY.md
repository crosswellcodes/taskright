...
- `shared/specs/CREATE_FLOW_ASSIGNMENT.md` — 📋 **PLANNED (July 10, 2026)**: optional team/group assignment inside the service create flow (all upcoming visits), plus a service-level fan-out endpoint. **No migration** — reuses `service_assignments` (upsert per `selection_cycle_id`); dispatch view (`ForecastDayScreen`) preserved unchanged. Two entry points, one model. Sync targets when built: `API_REFERENCE.md` (create `assignee` field + new `PUT .../services/:id/assignment`), `HANDOFF.md`. Deferred: team labor costing (D3 gap).
