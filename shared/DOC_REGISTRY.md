...
- `shared/specs/TEAM_LABOR_COSTING.md` — 📋 **PLANNED (July 13, 2026)**: closes the D3 gap — group-assigned Calls record no labor → profitability blank on the cost side. **Option A**: a group assignment resolves to *all its members individually*; broaden the four member-facing resolvers (`getJobsForTeamMember`/`getJobDetail`/`completeJobForTeamMember`/`recordGeofenceEvent`) from `team_member_id`-only to *individual OR `team_memberships`*, reusing the per-member labor machinery wholesale. Per-member existing rate (TL2); first-to-complete-wins (TL3). **No migration.** Sync targets when built: `JOB_COSTING.md` + `JOB_COSTING_DATA_GAPS.md` (D3 → Option A shipped), `CREATE_FLOW_ASSIGNMENT.md` (D7 gap closed), `API_REFERENCE.md` (`getJobsForTeamMember` includes team jobs; no new endpoints), `HANDOFF.md`, mobile (remove D3 empty-state + create-flow group note).
