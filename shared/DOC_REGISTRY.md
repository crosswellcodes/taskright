...
- `shared/specs/GEOFENCE_TRANSPARENCY.md` — ✅ **BUILT (July 23, 2026)**, all three tiers. Makes team-member auto clock-in legible and recoverable. **Tier A (`JobDetailScreen.js`, mobile-only):** fixed the reactivation dead state — geofence effect re-arms per `selectionCycleId`, permission resolved via `getCurrentPosition` (reliable when already-authorized on iOS) instead of `requestAuthorization`'s unreliable callbacks; the two conditional cards collapse into one always-present "Time Tracking" card over all six `trackingMode` states (§3) so nothing renders blank; + on-the-clock elapsed timer, Enable-location/Settings path with `AppState` re-arm, and a leave-while-clocked-in `beforeRemove` confirmation (A3). Decisions GT-A1/A2/A3. **Tier B:** `getJobsForTeamMember` gains derived `autoTrackable` (+ route now passes `isTeamAssigned`/`teamName`, previously dropped) → `MyJobsScreen` auto/manual chip. **Tier C:** read-only `GET /team-members/:id/active-clock` (`getActiveClockForTeamMember`, `DISTINCT ON` over `geofence_events`) → `useActiveClock` hook + persistent `ClockedInBanner` above My Jobs (GT-C1/C2/C3). **No migration.** Synced: `API_REFERENCE.md` (`autoTrackable` + `active-clock` endpoint), `HANDOFF.md`, `FEATURE_MAPPING.md` (Team Time Tracking). **176/176 backend tests** (+11, `geofenceTransparency.test.js`).
- `shared/specs/SIGNALHOUSE_API_REFERENCE.md` — 🔲 **REFERENCE / NOT BUILT (captured July 26, 2026)**: user-validated capture of the SignalHouse SMS API for the planned **Twilio → SignalHouse provider migration**. Covers SDK conventions (errors returned-not-thrown, 11-digit no-`+` phone format, microdollars), the Group→Subgroup→Brand→Campaign→Number ISV model (maps 1:1 onto our Twilio architecture), full request+response shapes for auth (`/auth`, `/user/serviceuser` 365d key), provisioning (`/subgroup`, `/brand` async 202 + `ein`-echo scrub, `/campaign` autofill), numbers (`/number/available`, `/number` async), messaging (`/message/sms`, `/message/mms`, `enqueuedCount` semantics), and webhooks (`/webhook`, `MESSAGE_RECEIVED` payload, full event enum, **signature = configured outbound auth not HMAC**). Includes the `businesses`→payload field mapping, the provider-neutral `sms_*` DB migration plan, event-driven provisioning sequence, open decisions (§9: sole-prop entityType, new `vertical` field, usecase), and a **P0–P6 phased execution plan** behind a single `SmsProvider` seam. **No code yet.** Next: write the migration *design spec* from this + `HANDOFF.md`. Sync targets on build: `HANDOFF.md` (Twilio→SignalHouse decisions + `sms_*` DB columns), `API_REFERENCE.md` (inbound webhook route + any provisioning routes), `PRODUCT_OVERVIEW.md`/`FEATURE_MAPPING.md` (SMS provider).
- `shared/specs/SIGNALHOUSE_MIGRATION.md` — 🟡 **DESIGN + P0–P5 BUILT, live gating pending (July 31, 2026)**: the migration *design spec* — `SmsProvider` seam interface, per-file swap list, dual-provider env-flag strategy (`SMS_PROVIDER` + per-capability overrides), `sms_*` DB migration, OTP self-build, event-driven provisioning, per-phase test plan, and **§10 Go-Live/Live-Credentials Checklist (single source of truth for anything needing live creds)**. **Built & backend-green (216/216):** P0 seam + `TwilioProvider` wrapping existing Twilio; P1 migration `026` (`twilio_*`→`sms_*` renames + `vertical`/`sms_provider`, kept `twilio_messaging_service_sid` until P6); P2 `SignalHouseProvider.send`; P3 inbound (`parseInbound`/`verifyInboundSignature`/`fetchInboundMedia`/Global webhook); P4 provisioning + A2P (`provisionBusiness`/`registerA2P`/`handleWebhookEvent`, EIN-scrub); P5 OTP self-build (migration `027_otp_codes`). Providers under `backend/src/services/sms/`; SDK `@signalhousellc/sdk`. Ops scripts `backend/scripts/signalhouse-{whoami,smoke}.js`. **Live status:** token+group `G000045V` verified live; free control-plane (subgroup/number-search) confirmed; **Brand+ needs funded wallet** (mock isn't free). **Not cut over — Twilio still default** (`SMS_PROVIDER=twilio`); P6 (retire Twilio) pending live P2–P5 verification. §8 decisions locked: sole-prop→`PRIVATE_PROFIT`+EIN, `vertical=PROFESSIONAL`, `usecase=CUSTOMER_CARE`. Sync targets: `HANDOFF.md` (`sms_*` schema + pending-work status), `API_REFERENCE.md` (webhook route unchanged path), this file.
