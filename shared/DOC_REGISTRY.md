...
- `shared/specs/GEOFENCE_TRANSPARENCY.md` — 🔨 **TIER A BUILT (July 23, 2026)**; B & C still SPEC. Makes team-member auto clock-in legible and recoverable. **Tier A (`JobDetailScreen.js`, mobile-only, no backend/migration):** fixed the reactivation dead state — geofence effect re-arms per `selectionCycleId`, permission resolved via `getCurrentPosition` (reliable when already-authorized on iOS) instead of `requestAuthorization`'s unreliable callbacks; the two conditional cards collapse into one always-present "Time Tracking" card over all six `trackingMode` states (§3) so nothing renders blank; + on-the-clock elapsed timer, Enable-location/Settings path with `AppState` re-arm, and a leave-while-clocked-in `beforeRemove` confirmation (A3). Decisions GT-A1/A2/A3. **Tier B (pending):** `getJobsForTeamMember` gains `autoTrackable` → My Jobs auto/manual chip. **Tier C (pending):** read-only `GET /team-members/:id/active-clock` derived from `geofence_events` → cross-screen "you're clocked in" banner. Sync targets: `HANDOFF.md` (milestone; B/C add API_REFERENCE + payload/endpoint), `FEATURE_MAPPING.md` (Team Time Tracking).
