...
| **Team Time Tracking (clock-in/out)** | ✅ Auto geofence + manual fallback | ❌ Mobile-only | P1 | Auto clock-in needs a geocoded customer address; reliability hardened July 20 (relevance-gated geocode + bounded retry + owner "couldn't map this address" flag). Transparency Tier A (July 23): unified always-present "Time Tracking" card fixes the reactivation dead state, adds an on-the-clock elapsed timer + Enable-location retry + leave-while-clocked-in confirmation. Tiers B (My Jobs auto/manual chip) & C (cross-screen "clocked in" banner) pending. See `GEOCODING_RELIABILITY.md`, `GEOFENCE_TRANSPARENCY.md`. |
