...
| **Team Time Tracking (clock-in/out)** | ✅ Auto geofence + manual fallback | ❌ Mobile-only | P1 | Auto clock-in needs a geocoded customer address; reliability hardened July 20 (relevance-gated geocode + bounded retry + owner "couldn't map this address" flag). See `GEOCODING_RELIABILITY.md`. |
